...
class TensorboardLogger:
    ...
    def loop_train_log(self, *args, **kwargs) -> None:
        params = {'type': 'train'}
        kwargs.update(params)
        self.loop_log(*args, **kwargs)
    
    def loop_val_log(self, *args, **kwargs) -> None:
        params = {'type': 'val'}
        kwargs.update(params)
        self.loop_log(*args, **kwargs)

    def loop_log(self, progress_bar, dim: int, epoch: int, batch_idx: int,
                 inputs, labels, preds, probs, attn_maps, loss, type: str) -> None:
        """
        ...
        if (batch_idx + 1) % self.step == 0:
            ...
            self.writer.add_scalar(f"Loss/{type}", self.avg_training_loss/self.step, global_step)

            # Log input images as a grid
            img_grid = vutils.make_grid(inputs[:self.max_grid_dim], normalize=True)
           
            self.writer.add_image(f"Input Images/{type}", img_grid, global_step)

            if attn_maps is not None:
                attn_grid = vutils.make_grid(attn_maps[:self.max_grid_dim], normalize=True)
                self.writer.add_image(f"Attention Maps/{type}", attn_grid, global_step)
            ...
            for i in range(min(self.max_grid_dim, inputs.size(0))):
                ...
                log_text = f"True: {true_label}, Pred: {pred_label}, Max Prob: {max_prob:.4f}/{type}"
                self.writer.add_text(f"Labels_and_Probabilities for Image_{i}/{type}", log_text, global_step)
